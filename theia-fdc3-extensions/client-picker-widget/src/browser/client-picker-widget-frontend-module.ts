...
export default new ContainerModule(bind => {
    ...
    bind(FrontendApplicationContribution).toService(ClientPickerWidgetContribution);
    bind(ClientPickerWidgetWidget).toSelf();
    bind(WidgetFactory).toDynamicValue(ctx => ({
        id: ClientPickerWidgetWidget.ID,
        createWidget: () => ctx.container.get<ClientPickerWidgetWidget>(ClientPickerWidgetWidget)
    })).inSingletonScope();
});
